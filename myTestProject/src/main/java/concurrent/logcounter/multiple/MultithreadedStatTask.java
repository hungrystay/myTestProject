...
import concurrent.logcounter.common.AbstractStatTask;
import concurrent.logcounter.common.StatProcessor;
...
public class MultithreadedStatTask extends AbstractStatTask {
  ...
  /* 实例初始化块 */
  {
    ...
    batchSize = null != strBatchSize ? Integer.valueOf(strBatchSize) : 1000;
  }
  ...
  protected void doCalculate() throws IOException, InterruptedException {
    final LogReaderThread logReaderThread = createLogReader();
    // 启动工作者线程
  ...
  protected LogReaderThread createLogReader() {
    LogReaderThread logReader = new LogReaderThread(in, inputBufferSize,
        batchSize);
    return logReader;
  }
}